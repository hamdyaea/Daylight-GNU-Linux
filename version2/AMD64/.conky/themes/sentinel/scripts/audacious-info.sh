...
DrawBG(){
    convert -size ${Width}x${Height} xc:${BGColor} \
        ...
        -alpha on -channel RGBA -evaluate multiply ${Opacity} \
         ~/.Conky/sentinel/images/audbg.png
}
...
GetArt(){
    ...
    if [ -f "$FilePath"/"$AlbumArt" ];then
        cp "$FilePath"/"$AlbumArt" ~/.Conky/sentinel/images/
    else
        cp ~/.Conky/sentinel/images/"$BackUpArt" ~/.Conky/sentinel/images/"$AlbumArt"
    fi
}
...
# if 
    AudaciousInfo bg
    AudaciousInfo art
    echo -n "\${image ~/.Conky/sentinel/images/audbg.png -p 0,0}"
    echo -n "\${image ~/.Conky/sentinel/images/"$AlbumArt" -p 9,9 -s 100x100}"
    echo ""
    echo -n "                 "
